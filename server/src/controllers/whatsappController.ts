...
export const sendMedia = async (req: Request, res: Response) => {
  try {
    ...
    const uploadRes = await fetch(`https://graph.facebook.com/v18.0/${phoneId}/media`, {
      method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: uploadForm as any
    });
    const uploadData: any = await uploadRes.json();
    if (!uploadData.id) return res.status(500).json({ error: 'Media upload failed', detail: uploadData });
    const mediaId = uploadData.id;
    // Send message
    const msgBody: any = { messaging_product: 'whatsapp', to, type: waType,
      [waType]: waType === 'document' ? { id: mediaId, filename: file.originalname } : { id: mediaId }
    };
    ...
    const messageId = sendData.messages[0].id;
    const lead = await findLeadByPhone(to);
    const msgText = waType === 'document' ? `[document:${mediaId}:${file.originalname}:${mime}]`
      : waType === 'image' ? `[image:${mediaId}]`
      : waType === 'video' ? `[video:${mediaId}]` : `[audio:${mediaId}]`;
    await db.query(
      `INSERT INTO whatsapp_messages (message_id, from_number, to_number, message_text, direction, status, contact_name, timestamp)
       VALUES ($1, $2, $3, $4, 'outbound', 'sent', $5, NOW()) ON CONFLICT (message_id) DO NOTHING`,
      [messageId, phoneId, to, msgText, contactName || lead?.contact_name || to]
    );
    res.json({ success: true, messageId, mediaId });
  } catch (err: any) {
