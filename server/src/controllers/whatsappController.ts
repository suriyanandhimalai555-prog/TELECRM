...
export const getConversations = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (page && pageSize) {
      var countResult = await db.query('SELECT COUNT(*) FROM (' + query + ') as countsub', params);
      var total = parseInt(countResult.rows[0].count, 10);
      var offset = (page - 1) * pageSize;
      var pagedParams = params.slice();
      pagedParams.push(pageSize, offset);
      var pagedQuery = query + ' LIMIT $' + (pagedParams.length - 1) + ' OFFSET $' + pagedParams.length;
      var pagedRows = await db.query(pagedQuery, pagedParams);
      res.set('X-Total-Count', String(total));
      return res.json({ conversations: pagedRows.rows });
    }
