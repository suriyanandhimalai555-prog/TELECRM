...
export const getLeads = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (page && pageSize) {
      ...
      var pagedQuery = baseQuery + ' LIMIT $' + (pagedParams.length - 1) + ' OFFSET $' + pagedParams.length;
      var pagedResult = await db.query(pagedQuery, pagedParams);
      res.set('X-Total-Count', String(total));
      return res.json(pagedResult.rows);
    }
