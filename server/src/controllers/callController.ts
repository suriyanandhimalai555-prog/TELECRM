...
export const getCalls = async (req: AuthRequest, res: Response) => {
  ...
  try {
    let query = `
      SELECT c.*, u.name as agent_name, u.phone as agent_phone, l.contact_name as lead_name, l.mobile as lead_mobile, l.whatsapp as lead_whatsapp, cp.name as campaign_name
      FROM calls c
    ...
    if (req.query.lead_id) {
      whereClauses.push(`c.lead_id = $${queryParams.length + 1}`);
