...
export default function CompaniesPage() {
  ...
  const expandCompany = async (id: number) => {
    ...
    if (!isOpen) {
      if (!waAccounts[id]) {
        const acc = await apiGet<WhatsAppAccount[]>(`/api/companies/${id}/whatsapp`).catch(() => []);
        setWaAccounts(prev => ({ ...prev, [id]: acc }));
      }
      if (!companyAdmins[id]) {
        const admins = await apiGet<CompanyAdmin[]>(`/api/users?company_id=${id}`).catch(() => []);
        setCompanyAdmins(prev => ({ ...prev, [id]: admins.filter((u: CompanyAdmin) => u.role === 'company_admin') }));
      }
    }
  };
  ...
  const addWAAccount = async (compId: number) => {
    if (!waForm.label || !waForm.phone_number_id || !waForm.access_token) {
      alert('Label, Phone Number ID and Access Token are required.');
      return;
    }
    setLoading(true);
  ...
  const addCompanyAdmin = async (compId: number) => {
    ...
    setLoading(true);
    try {
      await apiPost('/api/users', { ...adminForm, role: 'company_admin', company_id: compId });
      const admins = await apiGet<CompanyAdmin[]>(`/api/users?company_id=${compId}`);
      setCompanyAdmins(prev => ({ ...prev, [compId]: admins.filter((u: CompanyAdmin) => u.role === 'company_admin') }));
      ...
      {companies.map(c => (
        <div key={c.id} className="border border-gray-200 rounded-xl mb-3 overflow-hidden bg-white shadow-sm">
          <div className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => expandCompany(c.id)}>
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-black text-sm">{c.company_name.charAt(0).toUpperCase()}</div>
            ...
            <div className="flex items-center gap-2">
              <button onClick={e => { e.stopPropagation(); deleteCompany(c.id); }} className="p-1.5 rounded-lg text-red-400 hover:bg-red-50 hover:text-red-600 transition-colors"><Trash2 size={15} /></button>
            ...
            <div className="border-t border-gray-100 bg-gray-50 p-4 grid gap-4">
              <div>
                ...
                {(companyAdmins[c.id] || []).map(admin => (
                  <div key={admin.id} className="flex items-center gap-2 py-1.5 border-b border-gray-100 last:border-0">
                    <div className="w-6 h-6 rounded-full bg-purple-100 text-purple-600 font-black text-xs flex items-center justify-center">{admin.name.charAt(0)}</div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setShowAddWA(showAddWA === c.id ? null : c.id)} className="text-xs font-bold text-green-600 hover:text-green-700 flex items-center gap-1"><Plus size={13} /> Add Account</button>
                </div>
                {(waAccounts[c.id] || []).length === 0 && showAddWA !== c.id && <p className="text-xs text-gray-400 italic">No WhatsApp accounts yet.</p>}
                {(waAccounts[c.id] || []).map(acc => (
                  <div key={acc.id} className="flex items-center justify-between py-1.5 border-b border-gray-100 last:border-0">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-green-100 flex items-center justify-center"><MessageSquare size={12} className="text-green-600" /></div>
                      <span className="text-sm font-bold text-gray-700">{acc.label}</span>
                      <span className="text-xs text-gray-400">{acc.phone_number}</span>
                    </div>
                ...
                {showAddWA === c.id && (
                  <div className="mt-2 bg-white rounded-lg border border-green-100 p-3 grid gap-2">
                    <input className="px-3 py-1.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-green-400" placeholder="Label (e.g. Sales Line)" value={waForm.label} onChange={e => setWaForm(p => ({ ...p, label: e.target.value }))} />
                    <input className="px-3 py-1.5 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-green-400" placeholder="Phone Number" value={waForm.phone_number} onChange={e => setWaForm(p => ({ ...p, phone_number: e.target.value }))} />
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => addWAAccount(c.id)} disabled={loading} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700">Save Account</button>
