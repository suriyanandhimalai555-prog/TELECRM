...
export default function Settings() {
  ...
  const tabs = [
    { id: 'profile', name: 'Profile', icon: UserIcon, roles: ['ADMIN', 'MANAGER', 'EMPLOYEE'] },
    { id: 'administration', name: 'Users', icon: Shield, roles: ['ADMIN', 'MANAGER'] },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 relative">
                            ...
                            {(user?.role === 'ADMIN' || (user?.role === 'MANAGER' && u.reporting_to === user.id)) && u.id !== user.id && (
                              <div className="flex items-center justify-end space-x-1">
                                ...
                                <motion.button 
                                  ...
                                  className="p-1.5 text-green-200 hover:text-aura-red hover:bg-aura-red/5 rounded-lg transition-all"
                                >
                                  <Trash2 size={14} />
                                </motion.button>
                              </div>
      ...
      <ConfirmationModal 
        ...
        title="Delete User?"
        message="This will permanently remove the user and their access to the system. This action cannot be undone."
      />
    </div>
  );
}
