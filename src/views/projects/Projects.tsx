...
export default function Projects() {
  ...
  const [formData, setFormData] = useState({
    ...
    default_owner_id: '' as string | number
  });
  const [showReassignConfirm, setShowReassignConfirm] = useState(false);
  const [reassignInfo, setReassignInfo] = useState<{ projectId: number; employeeId: string | number; affectedCount: number } | null>(null);
  ...
  const fetchData = useCallback(async (search?: string) => {
    try {
      const projRes = await api.get('/projects', { params: { search } });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { default_owner_id, ...baseFields } = formData;
      if (editingProject) {
        await api.put(`/projects/${editingProject.id}`, baseFields);
        const ownerChanged = default_owner_id && String(default_owner_id) !== String((editingProject as any).default_owner_id || '');
        if (ownerChanged) {
          const impactRes = await api.get(`/projects/${editingProject.id}/owner-impact`, { params: { employee_id: default_owner_id } });
          const affectedCount = impactRes.data.affectedCount || 0;
          if (affectedCount > 0) {
            setReassignInfo({ projectId: editingProject.id, employeeId: default_owner_id, affectedCount });
            ...
            await api.put(`/projects/${editingProject.id}/assign-owner`, { employee_id: default_owner_id, confirmBulkReassign: false });
          }
        }
      } else {
        await api.post('/projects', baseFields);
      }
      triggerFlash('white');
      setShowModal(false);
      setEditingProject(null);
  ...
  const handleConfirmReassign = async () => {
    ...
    try {
      await api.put(`/projects/${reassignInfo.projectId}/assign-owner`, { employee_id: reassignInfo.employeeId, confirmBulkReassign: true });
      triggerFlash('white');
      fetchData();
    } catch (error) {
      console.error('Failed to reassign leads');
    } finally {
      setReassignInfo(null);
    }
  };
  const handleDeclineReassign = async () => {
    if (!reassignInfo) return;
    try {
      await api.put(`/projects/${reassignInfo.projectId}/assign-owner`, { employee_id: reassignInfo.employeeId, confirmBulkReassign: false });
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {(user?.role === 'ADMIN' || user?.role === 'MANAGER') && (
          <motion.button 
            ...
            onClick={() => {
              setEditingProject(null);
              setFormData({ name: '', description: '', status: 'ACTIVE', default_owner_id: '' });
              setShowModal(true);
            }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <motion.button 
                      ...
                      onClick={() => {
                        ...
                        setFormData({
                          ...
                          default_owner_id: (project as any).default_owner_id || ''
                        });
                        setShowModal(true);
                      }}
        ...
        {projects.length === 0 && (
          <div className="col-span-full p-12 border-2 border-dashed border-gray-100 rounded-2xl text-center">
            {loading ? <div className="ui-standard-spinner mx-auto" /> : <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">No Projects Found</p>}
          </div>
        )}
      </div>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                {editingProject && (
                  <div>
                    ...
                    <select value={formData.default_owner_id} onChange={(e) => setFormData({...formData, default_owner_id: e.target.value})} className="w-full px-3 py-2 bg-gray-50 border border-gray-100 rounded-lg text-xs font-black uppercase focus:outline-aura-red">
                      <option value="">No default owner</option>
                      {users.filter(u => u.role === 'EMPLOYEE').map(u => (
                        <option key={u.id} value={u.id}>{u.name}</option>
                      ))}
                    </select>
                    <p className="text-[9px] font-bold text-gray-300 mt-1">New leads from this project auto-assign to this employee.</p>
                  </div>
                )}
      ...
      <ConfirmationModal
        ...
        onClose={() => { setShowReassignConfirm(false); handleDeclineReassign(); }}
        onConfirm={handleConfirmReassign}
        title="Reassign Existing Leads?"
        message={reassignInfo ? `This project has ${reassignInfo.affectedCount} lead(s) currently owned by other employees. Reassign all of them to the new owner too?` : ''}
        confirmText="Yes, Reassign All"
        cancelText="No, Future Leads Only"
        variant="warning"
      />
    </div>
  );
