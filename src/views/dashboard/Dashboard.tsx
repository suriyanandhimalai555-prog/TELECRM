...
export default function Dashboard() {
  ...
  if (!stats) return <div className="p-20 text-center font-black text-aura-red uppercase tracking-widest">Data stream interrupted</div>;

  const statCards = [
    { name: 'Total Calls', value: stats.totalCalls, icon: Phone, color: 'bg-aura-red', filter: 'all' },
    { name: 'Connected', value: stats.connectedCalls, icon: CheckCircle, color: 'bg-green-600', filter: 'CONNECTED' },
    { name: 'Rejected', value: stats.notConnectedCalls, icon: XCircle, color: 'bg-gray-400', filter: 'FAILED' },
    { name: 'Interactions', value: stats.whatsappInteractions, icon: MessageSquare, color: 'bg-aura-red', filter: 'whatsapp' },
  ];
    ...
    <div className="space-y-8 relative pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] mt-2 ml-1">Live Operational Framework // Ver 4.0</p>
        </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((card, index) => (
          <motion.div 
            ...
            onClick={() => navigate(`/calls?status=${card.filter}`)}
