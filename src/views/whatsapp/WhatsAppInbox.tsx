...
const ACCOUNT_PHONE_IDS: Record<number, string> = {
  ...
  3: "1070621209476657",  // WA4 - India (ALMANZAR Primetech LLC)
  4: "1236875606179466",  // WA5 - Almanzar Dubai (+971 56 345 0844)
};
...
export default function WhatsAppInbox({ accountIndex = 0 }: WhatsAppInboxProps) {
  ...
  const accountLabel = accountIndex === 0 ? 'WhatsApp' : accountIndex === 1 ? 'WhatsApp 2' : accountIndex === 2 ? 'WhatsApp 3' : accountIndex === 3 ? 'WhatsApp 4' : 'WhatsApp 5';
  const accountColor = accountIndex === 0 ? 'text-blue-600' : accountIndex === 1 ? 'text-green-600' : accountIndex === 4 ? 'text-orange-600' : 'text-purple-600';
    ...
    <div className="flex h-[calc(100vh-140px)] bg-white rounded-[2rem] shadow-2xl border border-gray-100 overflow-hidden">
      <div className={cn("w-full md:w-80 lg:w-96 border-r border-gray-100 flex flex-col bg-gray-50/50 shrink-0", selectedContact && "hidden md:flex")}>
        <div className="p-4 border-b border-gray-100 bg-white">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-0.5">
              <button
                onClick={() => {
                  if (accountIndex === 3) { navigate('/app/whatsapp5'); return; }
                  if (accountIndex === 4) { navigate('/app/whatsapp4'); return; }
                  const routes = ['/app/whatsapp','/app/whatsapp2','/app/whatsapp3','/app/whatsapp4'];
                  navigate(routes[(accountIndex + 1) % 4]);
                }}
              ...
              >
                {accountIndex === 2 ? 'WA 3 → WA 4' : accountIndex === 3 ? 'WA 4 → WA 5' : accountIndex === 4 ? 'WA 5 → WA 4' : `WA ${accountIndex + 1} → WA ${accountIndex + 2}`}
              </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {convTotal !== null && conversations.length < convTotal && (
            <div className="p-4 flex justify-center">
              <button onClick={handleLoadMoreConversations} disabled={loadingMoreConvs}
                className="px-4 py-2 bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-100 disabled:opacity-50">
                {loadingMoreConvs ? 'Loading...' : `Load More (${conversations.length} of ${convTotal})`}
              </button>
            </div>
          )}
        </div>
