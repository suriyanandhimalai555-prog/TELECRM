...
interface StateDashboardStats {
  ...
  dailyTasks: number;
  recentCalls: any[];
  callTypeBreakdown: any[];
}

export default function StateDashboardHome() {
  useOutletContext<{ user: any }>();
  const [stats, setStats] = useState<StateDashboardStats | null>(null);
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const res = await axios.get('/api/state/reports/dashboard-stats', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setStats(res.data);
    } catch (error: any) {
      const msg = error?.response?.data?.message || error?.message || 'Unknown error';
      console.error('Failed to fetch state stats:', msg);
      setError(msg);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-screen space-y-4 bg-gray-50">
      ...
      <p className="text-[10px] font-black text-aura-red uppercase tracking-widest animate-pulse">
        Initializing State CRM Interface...
      </p>
    </div>
  );
  ...
  const statCards = [
    { name: 'Total Contacts', value: stats.totalContacts || 0, icon: Users, color: 'bg-aura-red', path: '/state-crm/leads' },
    { name: 'Sent Today', value: stats.messagesToday || 0, icon: PhoneOutgoing, color: 'bg-[#2a85cc]', path: '/state-crm/whatsapp' },
    { name: 'Unread WA', value: stats.unreadWhatsAppCount || 0, icon: MessageSquare, color: 'bg-[#2a85cc]', path: '/state-crm/whatsapp' },
    { name: 'Calls Made', value: stats.totalCalls || 0, icon: Phone, color: 'bg-aura-red', path: '/state-crm/calls' },
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 sm:gap-4">
        <div>
          <h1 className="text-2xl sm:text-4xl font-black text-gray-900 border-l-4 border-aura-red pl-3 sm:pl-4 uppercase tracking-tighter">
            STATE <span className="text-aura-red">DASHBOARD</span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
            ...
            <button
              onClick={() => navigate('/state-crm/calls')}
