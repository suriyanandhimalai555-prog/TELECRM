...
export default function StateReminders() {
  ...
  useEffect(() => {
    Promise.all([
      stateApi.get("/reminders/due"),
      stateApi.get("/reminders/upcoming"),
    ]).then(([dueRes, upRes]) => {
  ...
  const handleSnooze = async (id: number) => {
    ...
    await stateApi.put(`/reminders/${id}`, { next_followup: tomorrow.toISOString() });
          ...
          <div className="space-y-3">
            {due.map(r => (
              <div key={r.id} className="flex items-center justify-between p-4 bg-red-50 rounded-xl border border-red-100">
                <div>
                  <p className="text-[12px] font-black text-gray-900">{r.name}</p>
                  <p className="text-[10px] text-gray-500">{r.phone} • {r.status}</p>
                  <p className="text-[10px] text-red-500 font-bold mt-1">📅 {formatDate(r.next_followup)}</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => navigate(`/state-crm/leads?id=${r.id}`)}
              ...
              <tbody>
                {upcoming.map(r => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50">
                    <td className="py-3">
                      <p className="text-[11px] font-black text-gray-900">{r.name}</p>
                      <p className="text-[9px] text-gray-400">{r.phone}</p>
                    </td>
                    <td className="py-3">
                      <span className="text-[9px] font-black uppercase px-2 py-1 bg-blue-50 text-blue-600 rounded-full">{r.status}</span>
                    </td>
                    <td className="py-3">
