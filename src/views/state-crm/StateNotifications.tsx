...
export default function StateNotifications() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setTasks(tasksData.filter((t: any) => t.status !== 'DONE').slice(0, 20));
  ...
  const tabs = [
    { key: "all", label: "All", count: leads.length + tasks.length },
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-gray-400 text-sm mb-6">Stay updated on leads and tasks</p>
                  ...
                  {leads.map((lead: any) => (
                    <div key={lead.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[11px] font-black text-gray-900">{lead.name}</p>
                          <p className="text-[9px] text-gray-400">{lead.phone} • {lead.status}</p>
                        </div>
                      </div>
                ...
                <div className="space-y-2">
                  {tasks.map((task: any) => (
                    <div key={task.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                      ...
                      <span className="text-[9px] font-black uppercase px-2 py-1 rounded-full bg-yellow-50 text-yellow-600">{task.status}</span>
                    </div>
                  ))}
                </div>
              )}
