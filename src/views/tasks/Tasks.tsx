...
export default function Tasks() {
  ...
  const fetchTasks = useCallback(async (search?: string, pageNum: number = 1) => {
    try {
      const res = await api.get('/tasks', { params: { search, page: pageNum, pageSize: TASKS_PAGE_SIZE } });
      var totalHeader = res.headers && res.headers['x-total-count'];
      setTasksTotal(totalHeader ? parseInt(totalHeader, 10) : res.data.length);
      if (pageNum === 1) {
        setTasks(res.data);
      } else {
  ...
  const handleLoadMoreTasks = () => {
    ...
    fetchTasks(undefined, tasksPage + 1);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setTasksTotal(prev => (prev !== null ? prev - 1 : prev));
    } catch (error) {
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {tasksTotal !== null && tasks.length < tasksTotal && (
            <div className="p-4 flex justify-center">
              <button onClick={handleLoadMoreTasks} disabled={loadingMoreTasks}
                ...
                {loadingMoreTasks ? 'Loading...' : `Load More (${tasks.length} of ${tasksTotal})`}
              </button>
            </div>
          )}
        </div>
