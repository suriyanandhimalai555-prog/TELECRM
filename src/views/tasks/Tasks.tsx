...
export default function Tasks() {
  ...
  const filteredTasks = Array.isArray(tasks) ? tasks.filter(task => {
    ...
    const matchesSearch = 
      (task.lead_name && task.lead_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (task.notes && task.notes.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (task.type && task.type.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (task.user_name && task.user_name.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-1.5 bg-white border border-gray-100 focus:border-aura-red rounded-lg focus:outline-none transition-all text-xs font-bold"
            />
          </div>
          <select 
            value={selectedProject}
            onChange={(e) => setSelectedProject(e.target.value)}
            className="px-3 py-1.5 bg-gray-50/50 border border-transparent focus:border-aura-red rounded-lg text-[10px] font-black text-gray-500 hover:text-gray-700 uppercase appearance-none cursor-pointer outline-none shadow-sm"
          >
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {tasks.length === 0 && (
            <div className="p-10 border-2 border-dashed border-gray-100 rounded-2xl text-center">
              {loading ? <div className="anime-sharingan-spinner mx-auto" /> : <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">No tasks assigned</p>}
            </div>
          )}
        </div>
