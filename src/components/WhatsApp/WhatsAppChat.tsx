    ...
    <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-5 flex flex-col items-center border-b border-gray-100">
          <div className="w-16 h-16 rounded-full bg-[#25D366] flex items-center justify-center text-white font-black text-xl mb-3">
        ...
        <div className="p-4 space-y-2.5 border-b border-gray-100">
          {[
            ...
            <div key={label} className="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-xl">
              <Icon size={14} className="text-[#25D366] shrink-0" />
              <div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-black text-gray-900 uppercase tracking-tighter">
                Whats<span className="text-[#25D366]">App</span>
              </h2>
              {totalUnread > 0 && (
                <span className="px-1.5 py-0.5 bg-[#25D366] text-white text-[9px] font-black rounded-full leading-none">{totalUnread}</span>
              )}
            </div>
              ...
              <div className="relative" ref={filterRef}>
                <button onClick={() => setShowFilterMenu(v => !v)}
                  className={cn("p-2 rounded-lg transition-colors text-sm",
                    showFilterMenu ? "bg-green-50 text-[#25D366]" : "text-gray-400 hover:text-[#25D366] hover:bg-green-50")}>
                  <Filter size={15} />
                </button>
                <AnimatePresence>
                  {showFilterMenu && (
                      ...
                      {[
                        ...
                        <button key={val} onClick={() => { setActiveFilter(val); setShowFilterMenu(false); }}
                          className={cn("w-full flex items-center gap-2.5 px-4 py-2 text-xs font-bold transition-colors",
                            activeFilter === val ? "bg-green-50 text-[#25D366]" : "text-gray-700 hover:bg-gray-50")}>
                          <Icon size={13} />{label}
                        </button>
                      ))}
              ...
              <div className="relative" ref={dotsRef}>
                <button onClick={() => setShowDotsMenu(v => !v)}
                  className={cn("p-2 rounded-lg transition-colors",
                    showDotsMenu ? "bg-green-50 text-[#25D366]" : "text-gray-400 hover:text-[#25D366] hover:bg-green-50")}>
            ...
            {(['all', 'unread'] as const).map(f => (
              <button key={f} onClick={() => setActiveFilter(f)}
                className={cn("px-2.5 py-1 rounded-full text-[10px] font-black uppercase transition-colors",
                  activeFilter === f ? "bg-[#25D366] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200")}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-10 gap-3">
              <div className="w-7 h-7 border-2 border-green-200 border-t-[#25D366] rounded-full animate-spin" />
            ...
            filteredConversations.map(conv => (
              <motion.div key={conv.contact_number} whileHover={{ x: 2 }}
                ...
                <div className={cn(
                  ...
                  conv.last_direction === 'inbound' ? "bg-[#25D366]" : "bg-gray-400"
                )}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {(conv.unread_count || 0) > 0 && (
                      <span className="w-4 h-4 bg-[#25D366] text-white text-[8px] font-black flex items-center justify-center rounded-full shrink-0">
      ...
      <div className="flex-1 flex min-w-0">
        {selectedContact ? (
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="px-4 py-3 bg-white border-b border-gray-100 flex items-center justify-between shadow-sm z-10 shrink-0">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-[#25D366] flex items-center justify-center text-white font-black text-sm">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button onClick={() => setShowContactInfo(v => !v)}
                  className={cn("p-2 rounded-lg transition-colors", showContactInfo ? "bg-green-50 text-[#25D366]" : "text-gray-400 hover:bg-gray-100")}>
                  <Info size={17} />
                </button>
                {/* Chat menu */}
                <div className="relative" ref={chatMenuRef}>
                  <button onClick={() => setShowChatMenu(v => !v)}
                    className={cn("p-2 rounded-lg transition-colors", showChatMenu ? "bg-green-50 text-[#25D366]" : "text-gray-400 hover:bg-gray-100")}>
            ...
            <div className="px-4 py-3 bg-white border-t border-gray-100 shrink-0">
              <div className="flex items-end gap-2">
                <div className="flex items-center gap-1 shrink-0 pb-1.5">
                  <div className="relative" ref={emojiRef}>
                    <button onClick={() => setShowEmojiPicker(v => !v)}
                      className={cn("p-2 rounded-lg transition-colors", showEmojiPicker ? "text-[#25D366]" : "text-gray-400 hover:text-[#25D366]")}>
                      <Smile size={19} />
                    </button>
                    <AnimatePresence>
                      {showEmojiPicker && (
                  ...
                  <button onClick={() => setShowTemplateModal(true)} title="Templates"
                    className="p-2 text-gray-400 hover:text-[#25D366] rounded-lg transition-colors">
                    <Layout size={19} />
                  </button>
                  <button className="p-2 text-gray-400 hover:text-[#25D366] rounded-lg transition-colors">
                    <Paperclip size={19} />
                  </button>
                </div>
                ...
                <motion.button whileTap={{ scale: 0.9 }} onClick={handleSendMessage}
                  ...
                  className="shrink-0 w-10 h-10 bg-[#25D366] text-white rounded-full flex items-center justify-center disabled:opacity-40 shadow-md hover:bg-green-700 transition-colors">
                  {sending
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center bg-gray-50/50">
            ...
            <h3 className="text-base font-black text-gray-700 mb-1.5">Inbox <span className="text-[#25D366]">Inactive</span></h3>
              ...
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                <div>
                  <h3 className="text-base font-black text-gray-900 uppercase tracking-tight">
                    Message <span className="text-[#25D366]">Templates</span>
                  </h3>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={handleSyncTemplates} disabled={syncingTemplates}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-green-200 rounded-lg text-[10px] font-black text-[#25D366] hover:bg-green-50 disabled:opacity-50 transition-colors">
                  ...
                  {templates.length === 0 ? (
                    ...
                    <div key={tpl.id} onClick={() => setSelectedTemplate(tpl)}
                      ...
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[8px] font-black text-[#25D366] uppercase bg-green-50 px-1.5 py-0.5 rounded">{tpl.category}</span>
                ...
                <div className="flex-1 bg-gray-50 p-5 flex flex-col">
                            ...
                            {comp.type === 'BUTTONS' && (
                              <div className="mt-2 space-y-1">
                                {comp.buttons?.map((btn: any, bi: number) => (
                                  <div key={bi} className="text-center text-[10px] font-bold text-[#25D366] py-1 bg-green-50 rounded-lg">{btn.text}</div>
                                ))}
                              </div>
                            )}
                          </div>
                  ...
                  <div className="pt-4 border-t border-gray-200 flex justify-end">
                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                      ...
                      className="flex items-center gap-2 px-5 py-2 bg-[#25D366] text-white rounded-xl text-xs font-black shadow-lg disabled:opacity-50 hover:bg-green-700 transition-colors">
